...
- LIS Columns: If someone has LIS, then it's not guaranteed they have both Part A and Part B, as LIS usually only requires one or the other. However, LIS is an SEP for MA, so it might be worth using these columns as supplementary supporting data.
